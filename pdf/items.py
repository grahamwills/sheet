...
from .components import Checkboxes, TextField, TitledTable
...
def make_table(data, style_commands, colWidths=None):
...
def build_inner_table(sections, on_left, padding):
    if not sections:
        return None

    data = [[s.as_flow(on_left)] for s in sections]
    style_commands = [
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
        ('LEFTPADDING', (0, 0), (-1, -1), 0),
        ('RIGHTPADDING', (0, 0), (-1, -1), 0),
    ...
    return make_table(data, style_commands)
...
class Part():
    ...
    def as_flow(self, on_left):
        ...
        if not content:
            #TODO image
            return Paragraph("image goes here", style=self.text_style)
